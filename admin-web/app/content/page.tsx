            ...
            {(trend ?? []).map(tr => {
              ...
              return (
                <div key={tr.kind + tr.ref_id} className="flex items-center gap-3 border-b border-[#F0EFEC] px-5 py-2.5 text-[12.5px] last:border-0">
                  ...
                  <span className={'shrink-0 rounded-full px-2 py-0.5 text-[10.5px] font-bold ' + (tr.kind === 'post' ? 'bg-[#EEF2FB] text-[#17181C]' : 'bg-[#FBF4E4] text-[#B45309]')}>{tr.kind}</span>
                  <Link href={'/users/' + tr.user_id} className="shrink-0 font-semibold text-[#17181C] hover:underline">{a.full_name || '@' + (a.username || '?')}</Link>
          ...
          return (
            <div key={p.id} className="border-b border-[#F0EFEC] px-5 py-3.5 last:border-0">
              ...
              <div className="mt-2 flex items-center gap-3">
                ...
                <Link href={'/users/' + p.user_id} className="text-[11.5px] font-semibold text-[#17181C] hover:underline">Open author</Link>
