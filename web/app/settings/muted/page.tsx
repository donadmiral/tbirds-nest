  ...
  return (
    <div className="mx-auto max-w-[560px] px-1">
      ...
      : rows.length === 0 ? (
        ...
        <div key={r.muted_id} className="mb-2 flex items-center gap-3 rounded-xl border border-ink/10 p-3">
          {r.avatar_url ? (
            // eslint-disable-next-line @next/next/no-img-element
            <img src={r.avatar_url} alt="" className="h-11 w-11 rounded-full object-cover" />
          ) : <span className="flex h-11 w-11 items-center justify-center rounded-full bg-navy text-[13px] font-semibold text-white">{(r.full_name || "?").charAt(0)}</span>}
          <div className="min-w-0 flex-1">
            <p className="truncate text-[14px] font-semibold text-ink">{r.full_name}</p>
            {r.username ? <p className="text-[12px] text-ink/45">@{r.username}</p> : null}
          </div>
          <button onClick={() => unmute(r.muted_id)} className="rounded-full border border-ink/15 px-3.5 py-1.5 text-[12.5px] font-semibold text-ink">Unmute</button>
        </div>
      ))}
    </div>
  );
