  ...
  return (
    <div className="mx-auto max-w-[560px] px-1">
      ...
      : rows.length === 0 ? (
        ...
        <div key={r.id} className="mb-2 flex items-center gap-3 rounded-xl border border-ink/10 p-3">
          <Link href={"/" + (r.username || "")} className="flex min-w-0 flex-1 items-center gap-3">
            {r.avatar_url ? (
            ...
            <div className="min-w-0">
              <p className="truncate text-[14px] font-semibold text-ink">{r.full_name || "Member"}</p>
              <p className="text-[12px] text-ink/45">{r.username ? "@" + r.username + " · " : ""}{relTime(r.created_at)}</p>
            </div>
          </Link>
          <button onClick={() => respond(r.id, "accept")} disabled={!!busy[r.id]} className="rounded-full bg-ink px-3.5 py-1.5 text-[12.5px] font-semibold text-white disabled:opacity-40">Accept</button>
          <button onClick={() => respond(r.id, "reject")} disabled={!!busy[r.id]} className="rounded-full border border-ink/15 px-3 py-1.5 text-[12.5px] text-ink/60">Decline</button>
        </div>
      ))}
    </div>
  );
