...
export default function ApplicantsPage() {
  ...
  if (allowed === false) {
    return (
      <div className="px-1 py-16 text-center">
        <p className="text-[15px] text-white/70">Only the poster of this job can view its applicants.</p>
  ...
  return (
    <div className="px-1">
      <Link href={"/jobs/" + jobId} className="mb-4 inline-block text-sm text-white/50 hover:text-white">← {jobTitle || "Job"}</Link>
      ...
      <div className="mt-3 flex gap-2 overflow-x-auto pb-2">
        {["all", ...STATUS_ORDER].map((s) => (
          <button key={s}
            onClick={() => setFilter(s)}
            className={"shrink-0 rounded-md px-3 py-1.5 text-[13px] transition-colors " + (filter === s ? "bg-surface-elevated font-semibold text-white" : "bg-surface text-white/60 hover:text-white")}
          >
      ...
      {allowed === null ? (
        ...
        <p className="py-16 text-center text-sm text-white/40">{filter === "all" ? "Applications will appear here as they come in." : "Move applicants here from their status menu."}</p>
      ) : (
              ...
              <div key={a.id} className="rounded-lg border border-white/10 p-4">
                <div className="flex items-center gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    <Link href={a.profile?.username ? "/" + a.profile.username : "#"} className="block truncate text-[15px] font-semibold text-white hover:underline">
                      {a.profile?.full_name ?? "Member"}
                    </Link>
                    <p className="text-[12px] text-white/40">@{a.profile?.username} · {timeAgo(a.created_at || new Date().toISOString())}</p>
                  </div>
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  ...
                  {a.applicant_phone ? (
                    <span className="flex items-center gap-1.5 rounded-md bg-surface px-3 py-1.5 text-[13px] text-white/70"><Phone size={14} /> {a.applicant_phone}</span>
                  ) : null}
                  {a.portfolio_url ? (
                    <a href={a.portfolio_url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 rounded-md bg-surface px-3 py-1.5 text-[13px] text-white/80 hover:bg-surface-elevated">
                ...
                {scheduling === a.id ? (
                  <div className="mt-3 flex flex-col gap-2 rounded-md bg-surface p-3">
                    <input type="datetime-local" value={when} onChange={(e) => setWhen(e.target.value)} className="rounded-md bg-surface-elevated px-3 py-2 text-[13px] text-white outline-none" />
                    <input value={where} onChange={(e) => setWhere(e.target.value)} placeholder="Location or link" className="rounded-md bg-surface-elevated px-3 py-2 text-[13px] text-white placeholder:text-white/30 outline-none" />
                    <div className="flex gap-2">
                      ...
                      <button onClick={() => setScheduling(null)} className="rounded-md bg-surface-elevated px-4 py-2 text-[13px] text-white">Cancel</button>
                    </div>
                  </div>
                ) : null}
              </div>
