  ...
  return (
    <div className="px-1">
      ...
      {posts === null ? (
        ...
        <p className="py-16 text-center text-sm text-white/40">No posts with this topic yet.</p>
      ) : (
        posts.map((p) => (
          <Link key={p.id} href={"/post/" + p.id} className="flex gap-3 border-b border-white/10 px-1 py-4 transition-colors hover:bg-surface">
            ...
            <span className="min-w-0 flex-1">
              ...
              <span className="mt-0.5 block whitespace-pre-wrap text-[14px] text-white/85">
                <RichText text={(p.content ?? p.body ?? "").slice(0, 400)} />
              </span>
              <span className="mt-1 block text-[12px] text-white/40">{(p.likes_count ?? 0) + " likes · " + (p.comments_count ?? 0) + " comments"}</span>
            </span>
          </Link>
        ))
      )}
