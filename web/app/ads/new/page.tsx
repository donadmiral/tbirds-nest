...
export default function AdWizardPage() {
  ...
  const card = (on: boolean) => "flex items-center gap-3 rounded-xl border p-4 text-left transition-colors " + (on ? "border-pearl bg-surface" : "border-white/10 hover:bg-surface");
  ...
  return (
    <div className="px-1">
      <div className="flex items-center gap-3 pb-1">
        <Link href="/ads" className="rounded-full p-1.5 text-white/50 hover:bg-surface hover:text-white" title="Back to Ads"><ArrowLeft size={18} /></Link>
      ...
      <div className="flex gap-1.5 py-3">
        {[1, 2, 3, 4].map((s) => (
          <span key={s} className={"h-1 flex-1 rounded-full " + (s <= step ? "bg-pearl" : "bg-white/15")} />
        ))}
      </div>
      <h2 className="pb-4 text-[16px] font-semibold text-white">{stepTitle}</h2>

      {step === 1 ? (
        <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
          {GOALS.map((g) => (
            <button key={g.key} onClick={() => setGoal(g.key)} className={card(goal === g.key)}>
              <g.icon size={19} className={goal === g.key ? "text-pearl" : "text-white/50"} />
      ...
      {step === 2 ? (
        <div className="flex flex-col gap-5">
          <div>
            <p className="pb-2 text-[12px] font-semibold uppercase tracking-wide text-white/40">Write a new ad</p>
            <textarea value={newText}
              ...
              rows={2}
              className={"w-full resize-none rounded-xl border p-4 text-[14px] text-white placeholder:text-white/30 outline-none transition-colors " + (pick?.kind === "new" ? "border-pearl bg-surface" : "border-white/10")}
            />
          </div>
          {posts.length > 0 ? (
            <div>
              <p className="pb-2 text-[12px] font-semibold uppercase tracking-wide text-white/40">Or promote an existing post</p>
              <div className="flex flex-col gap-2">
                {posts.slice(0, 5).map((p) => (
                  <button key={p.id} onClick={() => setPick({ kind: "post", post: p })} className={card(pick?.kind === "post" && pick.post.id === p.id)}>
                    <span className="min-w-0 flex-1">
                      <span className="line-clamp-2 block text-[13px] text-white/85">{p.content || "Media post"}</span>
                      <span className="block text-[11px] text-white/40">{timeAgo(p.created_at)}</span>
                    </span>
          ...
          {listings.length > 0 ? (
            <div>
              <p className="pb-2 text-[12px] font-semibold uppercase tracking-wide text-white/40">Or a Market product, the ad builds itself</p>
              <div className="flex gap-2 overflow-x-auto pb-1">
                {listings.map((l) => {
                  const on = pick?.kind === "listing" && pick.listing.id === l.id;
                  return (
                    <button key={l.id} onClick={() => setPick({ kind: "listing", listing: l })} className={"relative w-28 shrink-0 overflow-hidden rounded-lg border text-left " + (on ? "border-pearl" : "border-white/10")}>
          ...
          {jobs.length > 0 ? (
            <div>
              <p className="pb-2 text-[12px] font-semibold uppercase tracking-wide text-white/40">Or a job, the ad builds itself</p>
              <div className="flex flex-col gap-2">
                {jobs.slice(0, 5).map((j) => (
                  <button key={j.id} onClick={() => setPick({ kind: "job", job: j })} className={card(pick?.kind === "job" && pick.job.id === j.id)}>
                    ...
                    <span className="min-w-0 flex-1">
                      <span className="block truncate text-[13px] font-semibold text-white">{j.title}</span>
                      <span className="block truncate text-[11px] text-white/40">{[j.company, j.location].filter(Boolean).join(" · ")}</span>
                    </span>
      ...
      {step === 3 ? (
        <div className="flex flex-col gap-5">
          <div>
            <p className="pb-2 text-[12px] font-semibold uppercase tracking-wide text-white/40">Audience</p>
            <div className={card(true)}>
              <span className="min-w-0 flex-1">
                <span className="block text-[14px] font-semibold text-white">Automatic</span>
                <span className="block text-[12px] text-white/50">Everyone eligible on Platinum Circles. Audience targeting arrives with Ads Manager.</span>
              </span>
              <Check size={16} className="text-pearl" />
            </div>
          </div>
          <div>
            <p className="pb-2 text-[12px] font-semibold uppercase tracking-wide text-white/40">Run for</p>
          ...
          <div>
            <p className="pb-2 text-[12px] font-semibold uppercase tracking-wide text-white/40">Impression cap, optional</p>
            <input value={cap}
      ...
      {step === 4 ? (
        <div className="flex flex-col gap-4">
          <div className="relative rounded-xl border border-white/10 p-4">
            <span className="absolute right-3 top-3 rounded-sm bg-surface px-1.5 py-0.5 text-[10px] font-bold uppercase tracking-wide text-white/50">Sponsored</span>
            <div className="flex gap-3">
              ...
              <div className="min-w-0 flex-1">
                <p className="text-[14px] font-semibold text-white">{me?.full_name}</p>
                <p className="mt-1 whitespace-pre-wrap text-[14px] text-white/85">{contentSummary}</p>
          ...
          <div className="rounded-xl border border-white/10 p-4 text-[13px] text-white/70">
            <p>Goal: <span className="font-semibold text-white">{GOALS.find((g) => g.key === goal)?.label}</span></p>
            <p className="mt-1">Audience: <span className="font-semibold text-white">Automatic</span></p>
            <p className="mt-1">Duration: <span className="font-semibold text-white">{days > 0 ? days + " days" : "Until stopped"}</span>{cap.trim() ? " · cap " + cap : ""}</p>
          </div>
      ...
      <div className="flex items-center gap-2 pt-6">
        {step > 1 ? (
          <button onClick={() => setStep(step - 1)} className="flex items-center gap-1.5 rounded-md bg-surface px-4 py-2.5 text-[13px] text-white"><ArrowLeft size={14} /> Back</button>
        ) : null}
        {step < 4 ? (
