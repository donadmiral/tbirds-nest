...
export function ListingCard({ l, saved: initiallySaved = false, viewerId = null }: { l: Listing; saved?: boolean; viewerId?: string | null }) {
  ...
  return (
    <Link href={"/market/" + l.id} className="overflow-hidden rounded-lg border border-ink/10 transition-colors hover:bg-surface">
      <span className="relative block aspect-square bg-surface">
        ...
        {l.condition ? (
          <span className="absolute left-2 top-2 rounded-sm bg-ink/70 px-1.5 py-0.5 text-[10px] font-bold text-ink">{l.condition}</span>
        ) : null}
        {viewerId ? (
          <button onClick={onHeart} title={saved ? "Unsave" : "Save"} className="absolute right-2 top-2 rounded-full bg-ink/60 p-1.5">
            <Heart size={15} className={saved ? "text-danger" : "text-ink/80"} fill={saved ? "currentColor" : "none"} />
          </button>
        ) : null}
        {sold ? (
          <span className="absolute inset-0 flex items-center justify-center bg-black/45 text-[13px] font-extrabold tracking-widest text-ink">
