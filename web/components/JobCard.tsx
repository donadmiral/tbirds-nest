  ...
  return (
    <Link href={"/jobs/" + job.id} className="flex gap-4 border-b border-white/10 px-1 py-5 transition-colors hover:bg-surface">
      ...
      <span className="min-w-0 flex-1">
        ...
        <span className="flex items-center gap-1.5 truncate text-[14px] text-white/70">{job.profile?.id ? <StoryAvatar userId={job.profile.id} name={job.profile.full_name} avatarUrl={job.profile.avatar_url} size={38} /> : null}{job.company}{job.verified ? <VerifiedBadge tier="business" size={13} /> : null}</span>
