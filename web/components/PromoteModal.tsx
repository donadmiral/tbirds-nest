...
export function PromoteModal({ postId, onClose }: { postId: string; onClose: () => void }) {
  ...
  const input = "rounded-md bg-surface px-3 py-2.5 text-[14px] text-ink placeholder:text-ink/30 outline-none";

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4" onClick={onClose}>
      <div onClick={(e) => e.stopPropagation()} className="w-full max-w-sm rounded-xl border border-ink/10 bg-navy p-5">
        <div className="flex items-center justify-between pb-3">
          <h2 className="flex items-center gap-1.5 text-[15px] font-semibold text-ink"><Megaphone size={16} className="text-pearl" /> Promote this post</h2>
        ...
        {done ? (
          ...
          <div className="flex flex-col gap-3">
            ...
            <p className="text-[11px] leading-relaxed text-ink/35">The post stays organic on your profile. The promotion additionally places it as Sponsored in eligible feeds. Pause or end it anytime from Ads. Billing and audience targeting arrive with Ads Manager.</p>
          </div>
        )}
      </div>
    </div>
