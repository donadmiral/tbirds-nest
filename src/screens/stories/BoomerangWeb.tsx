...
window.__bounce = async function(b64, mime){
  try {
    ...
    rec.start(200);
    var k=0;
    await new Promise(function(res){ var tick=function(){ ctx.drawImage(seq[k%seq.length],0,0); k++; if(k<=seq.length){ setTimeout(tick,1000/fps); } else { setTimeout(res,150); } }; tick(); });
    ...
    post({type:'done', b64:String(out).split(',')[1], mime:mimeOut, frames:seq.length, fps:fps});
  } catch(e) { post({type:'error', message:(e&&e.message)||String(e)}); }
};
post({type:'ready'});
</script></body></html>`;

export default function BoomerangWeb({ inputUri, onDone, onError }: { inputUri: string; onDone: (uri: string, durationSec: number) => void; onError: (message: string) => void }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        ref.current?.injectJavaScript(`window.__bounce(${JSON.stringify(b64)}, ${JSON.stringify(mime)}); true;`);
  ...
  const onMessage = async (ev: any) => {
    ...
    if (msg.type === 'done' && msg.b64) {
      ...
      try {
        ...
        onDone(dest, Math.max(1, Math.round((Number(msg.frames) || 48) / (Number(msg.fps) || 24))));
