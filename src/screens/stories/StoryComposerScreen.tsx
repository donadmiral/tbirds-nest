            ...
            <ScrollView keyboardShouldPersistTaps="handled" style={{ maxHeight: keyboard.keyboardHeight > 0 ? 150 : 220 }}>
              {mentionResults.map(u => { const picked = mentionPicks.some(p => p.id === u.id); return (
                <TouchableOpacity key={u.id} style={st.mentionRow} onPress={() => toggleMentionPick(u)} activeOpacity={0.6}>
                  ...
                  <View style={{ flex: 1 }}><View style={{ flexDirection: 'row', alignItems: 'center' }}><TierName text={u.full_name || u.username} tier={u.is_verified ? (u.verified_tier ?? 'business') : null} baseStyle={[st.mentionName, { flexShrink: 1, color: '#0B1E3D' }]} />{u.is_verified ? <View style={{ marginLeft: 5 }}><VerifiedBadge tier={u.verified_tier ?? undefined} userId={u.id} size={13} /></View> : null}</View>{u.username && <Text style={[st.mentionUsername, { color: 'rgba(11,30,61,0.55)' }]} numberOfLines={1}>@{u.username}</Text>}</View>
