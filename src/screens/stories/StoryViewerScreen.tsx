...
export default function StoryViewerScreen() {
  ...
  const panResponder = useMemo(() => PanResponder.create({
    ...
    onPanResponderMove: (_e, g) => { const ts = (_e as any)?.nativeEvent?.touches; if (ts && ts.length >= 2) { const dx = ts[0].pageX - ts[1].pageX; const dy = ts[0].pageY - ts[1].pageY; const dist = Math.sqrt(dx * dx + dy * dy); const midX = (ts[0].pageX + ts[1].pageX) / 2; const midY = (ts[0].pageY + ts[1].pageY) / 2; if (!pinchRef.current.active) { pinchRef.current = { active: true, startDist: dist, midX, midY }; if (longPressTimer.current) { clearTimeout(longPressTimer.current); longPressTimer.current = null; } didSwipe.current = false; translateY.setValue(0); pauseFor('pinch'); } const sc = Math.min(3, Math.max(1, dist / (pinchRef.current.startDist || dist))); pinchScale.setValue(sc); pinchTX.setValue((pinchRef.current.midX - SCREEN_W / 2) * (1 - sc)); pinchTY.setValue((pinchRef.current.midY - SCREEN_H / 2) * (1 - sc)); return; } if (pinchRef.current.active) return; if ((g.dy > 3 || g.dy < -3) && Math.abs(g.dy) > Math.abs(g.dx)) { if (longPressTimer.current) { clearTimeout(longPressTimer.current); longPressTimer.current = null; } didSwipe.current = true; if (g.dy > 0) { translateY.setValue(g.dy); } else { translateY.setValue(g.dy * RUBBER_BAND_FACTOR); } } },
      ...
      <Animated.View ref={mediaWrapRef} style={[s.mediaWrap, { transform: [{ translateX: pinchTX }, { translateY: pinchTY }, { scale: pinchScale }] }]} pointerEvents="box-none" onLayout={(e) => { handleMediaLayout(e); requestAnimationFrame(() => { (mediaWrapRef.current as any)?.measureInWindow?.((x: number, y: number) => { mediaOriginRef.current = { x: x || 0, y: y || 0 }; }); }); }}>
        ...
        {hasDual && isSplitMode ? renderSplitDual() : (
          ...
          <ReAnimated.View style={[StyleSheet.absoluteFill, activeMediaAnimStyle, { zIndex: 1 }]}>
            ...
            ) : isVideo ? (<>{mediaFitMode === 'contain' && (<View key={'vbg-' + currentStory.id} style={[s.media, { position: 'absolute', zIndex: -1 }]} pointerEvents="none"><BackgroundLayer bg={(mediaMt as any)?.bg || null} mediaUrl={null} /></View>)}<VideoView key={'v-' + currentStory.id} style={[s.media, mediaHasTransform && mediaMt ? { transform: [{ translateX: mediaMt.translateNX * mediaSize.w }, { translateY: mediaMt.translateNY * mediaSize.h }, { scale: mediaMt.scale }] } : undefined] as any} player={videoPlayer} contentFit={mediaFitMode} nativeControls={false} onFirstFrameRender={() => setMediaReady(true)} /></>
