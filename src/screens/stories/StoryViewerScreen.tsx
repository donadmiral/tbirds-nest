...
export default function StoryViewerScreen() {
  ...
  useEffect(() => { const u = isVideo && !isText ? currentStory?.media_url : null; if (u) { try { const p: any = videoPlayer; if (p?.replaceAsync) p.replaceAsync(u); else p?.replace?.(u); setTimeout(() => { try { p?.play?.(); } catch {} }, 50); } catch {} } }, [isVideo, isText, currentStory?.media_url, videoPlayer]);
