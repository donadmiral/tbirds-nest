...
export default function StoryViewerScreen() {
  ...
  const panResponder = useMemo(() => PanResponder.create({
    onStartShouldSetPanResponder: (e) => { const touchY = e.nativeEvent.pageY; const touchX = e.nativeEvent.pageX; if (touchY > SCREEN_H - BOTTOM_CONTROLS_HEIGHT - insets.bottom) return false; const pl = pollLayoutRef.current; const currentPoll = pollRef.current; if (pl && touchX >= pl.left && touchX <= pl.right && touchY >= pl.top && touchY <= pl.bottom) return false; if (!pl && currentPoll) { const estTop = Math.max(insets.top + 90, Math.min(currentPoll.ny * SCREEN_H - 100, SCREEN_H - 360)); const estLeft = (currentPoll.nx * SCREEN_W) - (SCREEN_W * 0.4); if (touchX >= estLeft && touchX <= estLeft + SCREEN_W * 0.8 && touchY >= estTop && touchY <= estTop + 260) return false; } if (touchInStickerRects(touchX, touchY)) { console.log('[veto] start yielded at', Math.round(touchX), Math.round(touchY)); return false; } return true; },
    onMoveShouldSetPanResponder: (_e, g) => { const x = (_e as any)?.nativeEvent?.pageX ?? 0; const y = (_e as any)?.nativeEvent?.pageY ?? 0; const still = Math.abs(g.dy) <= 14 && Math.abs(g.dx) <= 14; if (still && touchInStickerRects(x, y)) return false; /* veto only stills: real drags always dismiss */ return Math.abs(g.dy) > 10 || Math.abs(g.dx) > 10; },
  ...
  return (
    <Animated.View style={[s.root, { transform: [{ translateY }, { scale: translateY.interpolate({ inputRange: [-SCREEN_H * 0.3, 0, SCREEN_H], outputRange: [1, 1, 0.92], extrapolate: 'clamp' }) }] }]} {...panResponder.panHandlers}>
      ...
      <View ref={mediaWrapRef} style={s.mediaWrap} pointerEvents="box-none" onLayout={(e) => { handleMediaLayout(e); requestAnimationFrame(() => { (mediaWrapRef.current as any)?.measureInWindow?.((x: number, y: number) => { mediaOriginRef.current = { x: x || 0, y: y || 0 }; }); }); }}>
        ...
        <StickerOverlay stickers={stickers} containerW={mediaSize.w} containerH={mediaSize.h} onHashtagTap={(tag) => { navigation.navigate('Search' as never, { query: '#' + tag } as never); }} onMentionTap={(userId) => navigation.navigate('UserProfile', { userId })} onPostTap={(postId) => { (navigation as any).navigate('Post', { postId }); }} onStickerLayout={(id: string, r: any) => { measuredStickerRectsRef.current[id] = r; }} engagementProps={engagementProps} />
      </View>
      </Animated.View>
      </ReAnimated.View>
