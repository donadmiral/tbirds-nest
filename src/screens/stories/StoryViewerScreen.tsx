...
export default function StoryViewerScreen() {
  ...
  useEffect(() => { const stickerList = (currentStory?.stickers_json || []) as StoryTextSticker[]; const ox = mediaOriginRef.current.x; const oy = mediaOriginRef.current.y; const rects = stickerList.filter(s => s.kind === 'link' || s.kind === 'location' || s.kind === 'mention' || s.kind === 'hashtag' || s.kind === 'post').map(s => { const w = s.kind === 'post' ? POST_CARD_W : ((s.kind === 'mention' || s.kind === 'link') ? 170 : 150); const hEst = s.kind === 'post' ? POST_CARD_EST_H : 36; const cx = ox + s.nx * mediaSize.w; const cy = oy + s.ny * mediaSize.h; const hw = (w * s.scale) / 2 + 10; const hh = (hEst * s.scale) / 2 + 10; return { left: cx - hw, right: cx + hw, top: cy - hh, bottom: cy + hh }; }); tappableStickerRectsRef.current = rects; }, [currentStory?.stickers_json, mediaSize]);
      ...
      <Animated.View ref={mediaWrapRef} style={[s.mediaWrap, { transform: [{ translateX: pinchTX }, { translateY: pinchTY }, { scale: pinchScale }] }]} pointerEvents="box-none" onLayout={(e) => { handleMediaLayout(e); requestAnimationFrame(() => { (mediaWrapRef.current as any)?.measureInWindow?.((x: number, y: number) => { mediaOriginRef.current = { x: x || 0, y: y || 0 }; }); }); }}>
        ...
        {hasDual && isSplitMode ? renderSplitDual() : (
          ...
          <ReAnimated.View style={[StyleSheet.absoluteFill, activeMediaAnimStyle, { zIndex: 1 }]}>
            ...
            ) : isVideo ? (<VideoView key={'v-' + currentStory.id} style={s.media} player={videoPlayer} contentFit="cover" nativeControls={false} onFirstFrameRender={() => setMediaReady(true)} />
        ...
        <StickerOverlay stickers={stickers} containerW={mediaSize.w} containerH={mediaSize.h} onHashtagTap={(tag) => { saveAndGoBack(); setTimeout(() => navigation.navigate('Search' as never, { query: '#' + tag } as never), 280); }} onMentionTap={(userId) => { pauseFor('deepnav'); const un = (navigation as any).addListener('focus', () => { un(); resumeFrom('deepnav'); }); (navigation as any).push('UserProfileTop', { userId }); }} onPostTap={(postId) => { pauseFor('deepnav'); const un = (navigation as any).addListener('focus', () => { un(); resumeFrom('deepnav'); }); (navigation as any).push('PostTop', { postId }); }} onPostProfileTap={(uid: string) => { pauseFor('deepnav'); const un = (navigation as any).addListener('focus', () => { un(); resumeFrom('deepnav'); }); (navigation as any).push('UserProfileTop', { userId: uid }); }} onPostHoldStart={() => pauseFor('cardHold')} onPostHoldEnd={() => resumeFrom('cardHold')} onPostProfileTap={(uid: string) => { pauseFor('deepnav'); const un = (navigation as any).addListener('focus', () => { un(); resumeFrom('deepnav'); }); (navigation as any).push('UserProfileTop', { userId: uid }); }} onPostHoldStart={() => pauseFor('cardHold')} onPostHoldEnd={() => resumeFrom('cardHold')} onStickerLayout={(id: string, r: any) => { measuredStickerRectsRef.current[id] = r; }} engagementProps={engagementProps} />
      </Animated.View>{/* wrap */}
      </Animated.View>
      </ReAnimated.View>
