...
export default function StoryViewerScreen() {
  ...
  const panResponder = useMemo(() => PanResponder.create({
    ...
    onMoveShouldSetPanResponder: (_e, g) => { const x = (_e as any)?.nativeEvent?.pageX ?? 0; const y = (_e as any)?.nativeEvent?.pageY ?? 0; if (touchInStickerRects(x, y)) return false; return Math.abs(g.dy) > 10 || Math.abs(g.dx) > 10; },
