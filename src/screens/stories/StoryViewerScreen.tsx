...
export default function StoryViewerScreen() {
  ...
  const storyAudioUrl = currentStory && !isVideo ? ((currentStory as any).audio_url as string | null) || null : null;
  useStoryAudio(storyAudioUrl, !!storyAudioUrl && !!mediaReady && !paused);
  useEffect(() => { const stickerList = (currentStory?.stickers_json || []) as StoryTextSticker[]; const ox = mediaOriginRef.current.x; const oy = mediaOriginRef.current.y; const rects = stickerList.filter(s => s.kind === 'link' || s.kind === 'location' || s.kind === 'mention' || s.kind === 'hashtag' || s.kind === 'post' || s.kind === 'countdown').map(s => { const w = s.kind === 'post' ? POST_CARD_W : (s.kind === 'countdown' ? 236 : ((s.kind === 'mention' || s.kind === 'link') ? 170 : 150)); const hEst = s.kind === 'post' ? POST_CARD_EST_H : (s.kind === 'countdown' ? 150 : 36); const cx = ox + s.nx * mediaSize.w; const cy = oy + s.ny * mediaSize.h; const hw = (w * s.scale) / 2 + 10; const hh = (hEst * s.scale) / 2 + 10; return { left: cx - hw, right: cx + hw, top: cy - hh, bottom: cy + hh }; }); tappableStickerRectsRef.current = rects; }, [currentStory?.stickers_json, mediaSize]);
      ...
      <Animated.View ref={mediaWrapRef} style={[s.mediaWrap, { transform: [{ translateX: pinchTX }, { translateY: pinchTY }, { scale: pinchScale }] }]} pointerEvents="box-none" onLayout={(e) => { handleMediaLayout(e); requestAnimationFrame(() => { (mediaWrapRef.current as any)?.measureInWindow?.((x: number, y: number) => { mediaOriginRef.current = { x: x || 0, y: y || 0 }; }); }); }}>
        ...
        {hasDual && isSplitMode ? renderSplitDual() : (
          ...
          <ReAnimated.View style={[StyleSheet.absoluteFill, activeMediaAnimStyle, { zIndex: 1 }]}>
            ...
            ) : (<>{mediaFitMode === 'contain' && currentStory.media_url && (<ExpoImage key={'bg-' + currentStory.id} source={{ uri: currentStory.media_url }} style={[s.media, { position: 'absolute', zIndex: -1, opacity: 0.22 }]} contentFit="cover" blurRadius={35} transition={0} cachePolicy="memory-disk" />)}<ExpoImage key={'m-' + currentStory.id} source={hasDual && effectivePrimaryUrl ? { uri: effectivePrimaryUrl } : (currentStory.media_url ? { uri: currentStory.media_url } : undefined)} style={[s.media, mediaHasTransform && !hasDual ? { transform: [{ translateX: mediaMt!.translateNX * mediaSize.w }, { translateY: mediaMt!.translateNY * mediaSize.h }, { scale: mediaMt!.scale }] } : undefined]} contentFit={mediaFitMode} transition={0} cachePolicy="memory-disk" onLoad={() => setMediaReady(true)} onError={() => { setMediaError(true); setMediaReady(true); }} /></>)}
          </ReAnimated.View>
          </ZoomableMedia>
        )}
        </ReAnimated.View>

        <FilterLayer filterId={((currentStory as any)?.filter_id as string) || null} zIndex={2} />
        {renderPipDual()}
