...
export default function FeedScreen({ navigation }: any) {
  ...
  const [composerScope, setComposerScope] = useState<ScopeMode>('primary');
  ...
  // Scoped feed loader uses the get_scoped_feed RPC. Returns rows without
  // post_media joined, so we enrich them with a second query.
  const loadFeed = useCallback(async (showLoader = false) => {
    try {
      if (showLoader) setLoading(true);
      if (!userId) return;

      const { data: rpcRows, error: rpcErr } = await supabase.rpc('get_scoped_feed', {
        p_user_id: userId,
        p_mode: scopeMode,
        p_limit: 80,
        p_before: null,
      ...
      if (rpcErr) {
        console.log('[FEED] rpc error:', rpcErr.message);
        return;
      }
      const rows = rpcRows || [];

      // Enrich with post_media (batch fetch)
      const postIds = (rows as any[]).map(r => r.post_id).filter(Boolean);
      let mediaByPost: Record<string, MediaItem[]> = {};
      if (postIds.length > 0) {
        const { data: mediaRows } = await supabase
          .from('post_media')
          .select('id, post_id, url, media_type, width, height, sort_order')
          .in('post_id', postIds);
        (mediaRows || []).forEach((m: any) => {
          if (!mediaByPost[m.post_id]) mediaByPost[m.post_id] = [];
          mediaByPost[m.post_id].push(m);
        });
        Object.keys(mediaByPost).forEach(pid => {
          mediaByPost[pid].sort((a, b) => (a.sort_order ?? 0) - (b.sort_order ?? 0));
        });
      }

      const normalized = (rows as any[]).map((row: any): Omit<Post, 'score'> => ({
        id: row.post_id,
        user_id: row.user_id,
  ...
  const createPost = async () => {
    ...
    try {
      ...
      // Map composer scope to DB scope values. 'primary' maps to 'institution'
      // so the auto-fill trigger fills institution_id from the user's primary school.
      const dbScope = composerScope === 'primary' ? 'institution' : composerScope;
          ...
          <TouchableOpacity style={s.postMeta} onPress={() => navigation.navigate('UserProfile', { userId: post.user_id, user: author })} activeOpacity={0.8}>
            ...
            <View style={s.postMetaTxt}>
              ...
              <View style={s.postSubRow}>
                <Text style={s.postSub} numberOfLines={1}>
                  {author?.username ? `@${author.username}` : ''}
                  {author?.username && post.created_at ? ' · ' : ''}
                  {relTime(post.created_at)}
                </Text>
                {post.scope === 'global' ? (
                  <View style={s.scopeBadge}>
                    <Feather name="globe" size={10} color="#6B7280" />
                    <Text style={s.scopeBadgeTxt}>Global</Text>
                  </View>
                ) : post.institution_name ? (
                  <View style={s.scopeBadge}>
                    <Feather name="award" size={10} color="#1D4ED8" />
                    <Text style={[s.scopeBadgeTxt, { color: '#1D4ED8' }]} numberOfLines={1}>
                      {post.institution_name}
                    </Text>
                  </View>
                ) : null}
  ...
  const scopeLabel = (m: ScopeMode) =>
    m === 'primary' ? 'My School' : m === 'all' ? 'All Schools' : 'Global';
        ...
        <View style={s.container}>
          <View style={s.header}>
            ...
            <View style={s.scopeRow}>
              {(['primary', 'all', 'global'] as const).map(m => (
                <TouchableOpacity
                  key={m}
                  style={[s.scopeTab, scopeMode === m && s.scopeTabActive]}
                  onPress={() => setScopeMode(m)}
                  activeOpacity={0.7}
                >
                  <Feather
                    name={m === 'primary' ? 'award' : m === 'all' ? 'grid' : 'globe'}
                    size={12}
                    color={scopeMode === m ? '#FFFFFF' : '#6B7280'}
                  />
                  <Text style={[s.scopeTabTxt, scopeMode === m && s.scopeTabTxtActive]}>
                    {scopeLabel(m)}
                  </Text>
                </TouchableOpacity>
              ))}
          ...
          {loading ? (
            ...
            <FlatList
              ...
              ListHeaderComponent={<StoryStrip mode={scopeMode} />}
            ...
            <KeyboardAvoidingView style={[s.composerContainer, { bottom: insets.bottom + 16 }]} behavior={Platform.OS === 'ios' ? 'position' : undefined}>
              ...
              <View style={s.composerCard}>
                <View style={s.cAuthorRow}>
                  ...
                  <View style={{ flex: 1 }} />
                  <TouchableOpacity
                    style={s.cScopePicker}
                    onPress={() => setComposerScope(composerScope === 'primary' ? 'global' : 'primary')}
                    activeOpacity={0.7}
                  >
                    <Feather name={composerScope === 'primary' ? 'award' : 'globe'} size={12} color="#1D4ED8" />
                    <Text style={s.cScopePickerTxt}>
                      {composerScope === 'primary' ? 'My School' : 'Global'}
                    </Text>
                    <Feather name="chevron-down" size={12} color="#1D4ED8" />
                  </TouchableOpacity>
                </View>
...
const s = StyleSheet.create({
  ...
  scopeRow: { flexDirection: 'row', marginBottom: 8, gap: 6 },
  scopeTab: {
  ...
  scopeTabActive: { backgroundColor: '#000000', borderColor: '#000000' },
  ...
  postSubRow: { flexDirection: 'row', alignItems: 'center', gap: 6, marginTop: 2 },
  postSub: { fontSize: 12, color: '#8E8E93' },
  scopeBadge: {
  ...
  scopeBadgeTxt: { fontSize: 10, color: '#6B7280', fontWeight: '600' },
  ...
  cScopePickerTxt: { fontSize: 12, fontWeight: '700', color: '#1D4ED8' },
