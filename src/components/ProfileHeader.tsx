    ...
    <View>
      ...
      <View style={s.body}>
        ...
        <View style={s.nameRow}>
          <Text style={[s.name, getTierColor((profile as any)?.verified_tier ?? ((profile as any)?.is_verified ? 'business' : null)) ? { color: getTierColor((profile as any)?.verified_tier ?? ((profile as any)?.is_verified ? 'business' : null)) as string } : null]} numberOfLines={1}><TierName userId={((profile) as any)?.id ?? ((profile) as any)?.user_id} baseStyle={[s.name, getTierColor((profile as any)?.verified_tier ?? ((profile as any)?.is_verified ? 'business' : null)) ? { color: getTierColor((profile as any)?.verified_tier ?? ((profile as any)?.is_verified ? 'business' : null)) as string } : null]} text={profile?.full_name || 'Your Name' || ''} numberOfLines={1} /> <VerifiedBadge userId={((profile) as any)?.id ?? ((profile) as any)?.user_id} size={12} /></Text>
        ...
        {(() => { const c = (profile as any)?.account_class; const lb = (profile as any)?.account_labels || {}; const chips: string[] = []; if (c === 'creator') chips.push('Creator'); if (c === 'organization' || (profile as any)?.account_type === 'business') chips.push('Business'); if (lb?.parody) chips.push('Parody'); if (lb?.fan) chips.push('Fan account'); if (lb?.commentary) chips.push('Commentary'); if (lb?.memorialized) chips.push('Remembering'); if (!chips.length) return null; return (
