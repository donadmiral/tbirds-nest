...
export default function StudioRail({ items, top, maxHeight }: { items: RailItem[]; top: number; maxHeight: number }) {
  ...
  return (
    <View style={[r.wrap, { top }]} pointerEvents="box-none">
      <ScrollView style={[r.rail, { maxHeight: Math.max(120, maxHeight) }]} contentContainerStyle={r.railContent} showsVerticalScrollIndicator={false} bounces={false}>
        {shown.map(it => (
...
const r = StyleSheet.create({
  ...
  rail: { backgroundColor: 'rgba(12,16,26,0.58)', borderRadius: 24, borderWidth: StyleSheet.hairlineWidth, borderColor: 'rgba(255,255,255,0.18)' },
  railContent: { paddingVertical: 8, paddingHorizontal: 6, alignItems: 'center', gap: 2 },
