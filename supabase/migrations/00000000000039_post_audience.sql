-- 0039: post audience — everyone / followers / mentioned / verified
-- Enforced inside get_feed so visibility is decided by the database, not the phone.
-- Body-only change to get_feed (return list identical to 0014), so plain replace.
...
do $$ begin
  alter table public.posts
    ...
    check (audience in ('everyone','followers','mentioned','verified'));
