-- 0004_widen_for_you_window.sql
-- The 30 day candidate window starved For You on a quiet app: 39 of 47 posts
-- were ineligible and one author filled the feed. Recency decay already ranks
-- old posts down, so a hard cutoff was redundant. One year keeps the candidate
-- set bounded without hiding the archive.
...
scored as (
  select c.*, pr.full_name, pr.username, pr.avatar_url, pr.is_verified, pr.account_type,
    ...
    case when p_mode = 'for_you' then
      ...
      + (coalesce(c.reposts_count,0) * 2.0) + 1.0 )
    ...
    else extract(epoch from c.created_at) end as sort_key
  from candidates c
...
select s.id, s.user_id, s.content, s.body, s.media_url,
  ...
  s.liked, s.bookmarked, s.reposted, s.follows, s.sort_key
from scored s
where p_cursor_key is null or (s.sort_key, s.id) < (p_cursor_key, p_cursor_id)
