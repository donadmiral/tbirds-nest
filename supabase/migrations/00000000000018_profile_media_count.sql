-- 0018_profile_media_count.sql
-- get_profile gains a media count so the profile's Media tab can stop deriving
-- its badge from an ilike scan over every post body.
--
-- That scan was wrong twice over: it counted mentions rather than media, and
-- '%@don%' substring-matches '@donadmiral'. post_mentions exists and is indexed
-- if mentions are ever wanted again, but this tab is about media.
--
