-- 0044: link previews
-- link_previews caches OG metadata (written by the link-preview Edge Function
-- with the service role). posts.link_url holds the first URL in a post,
-- stamped by trigger like mentions. get_feed returns a `link` jsonb when a
-- cached preview exists. RETURN TYPE CHANGES so get_feed must be dropped first.
...
-- no insert/update policy for authenticated: only the Edge Function's
-- service role writes, so clients cannot poison the cache.
...
$tg$;
drop trigger if exists trg_sync_post_link on public.posts;
create trigger trg_sync_post_link before insert or update of content on public.posts
  for each row execute function sync_post_link();

update public.posts
set link_url = substring(coalesce(content, '') from 'https?://[^\s<>"]+')
where content ~ 'https?://' and link_url is null;
  ...
  select e.*,
    ...
    case
      ...
      when p_mode = 'trending' then
        e.velocity * (case when e.engagements >= 1 then 1.0 else 0.05 end)
...
diversified as (
  select b.*,
    case when p_mode in ('for_you', 'trending')
