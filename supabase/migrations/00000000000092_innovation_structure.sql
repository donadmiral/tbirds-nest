-- 0092 Innovation structure: an invention gets a FIELD and a STAGE.
-- Columns with checked values, and get_feed re-issued (return type grows,
-- so drop-then-create per the 0044 precedent) to carry both to the card.

alter table public.posts add column if not exists innovation_field text;
alter table public.posts add column if not exists innovation_stage text;
alter table public.posts drop constraint if exists posts_innovation_field_check;
alter table public.posts add constraint posts_innovation_field_check
  check (innovation_field is null or innovation_field in ('Agritech','Health','Energy','Fintech','Education','Other'));
alter table public.posts drop constraint if exists posts_innovation_stage_check;
alter table public.posts add constraint posts_innovation_stage_check
  check (innovation_stage is null or innovation_stage in ('Idea','Prototype','Launched'));
