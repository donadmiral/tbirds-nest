-- 0095 The badge travels with the post. get_feed re-issued from 0092 carrying
-- profiles.verified_tier as author_verified_tier, so every card can render the
-- author's metal. Return shape changes, so the function is dropped first.

drop function if exists public.get_feed(text, double precision, uuid, integer);
