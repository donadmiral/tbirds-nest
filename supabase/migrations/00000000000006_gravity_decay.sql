-- 0006_gravity_decay.sql
-- Replace exponential decay with polynomial (gravity) decay in For You.
--
-- exp(-hours/24) annihilates rather than ranks: a post from 80 days ago scored
-- 1.8e-35, which is indistinguishable from zero, so every author except the one
-- posting this week was mathematically erased from the feed.
--
-- score = engagement / (hours + 2) ^ 1.5
--
-- Recent content still wins comfortably, but a month-old post with real
-- engagement can still outrank a fresh post nobody touched, which is what a
-- feed should do. The +2 stops brand new posts dividing by almost zero.
