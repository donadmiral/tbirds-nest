-- 0084 The Trending page honors the badge. The chip and the page used two
-- different judges: chip = top-decile velocity within 72h with 3+ engagements,
-- page = raw velocity over 7 days. A badged post could wear the chip yet sit
-- buried on the page. This re-issues get_feed (extracted from 0044) with one
-- change: badge-holders rank first in trending mode, ordered by velocity among
-- themselves, then the rest of the week beneath.
  ...
  select e.*,
    ...
    case
      ...
      when p_mode = 'trending' then
        ...
        * (case when e.created_at > now() - interval '72 hours'
                 and e.engagements >= 3
                 and e.velocity >= (select cutoff from recent_velocity)
                 and (select cutoff from recent_velocity) > 0
