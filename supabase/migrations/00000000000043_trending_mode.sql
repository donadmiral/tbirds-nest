-- 0043: real Trending mode in get_feed + legacy media backfill
-- Trending ranks by engagement velocity over a 7-day window, with the author
-- diversity cap applied so one prolific author cannot own the tab, and a
-- floor so zero-engagement posts sit low rather than leading on recency.
-- Also backfills legacy posts.media_url rows into post_media (one-off).

insert into public.post_media (post_id, url, media_type, sort_order)
select p.id, p.media_url,
       case when p.media_url ilike '%.mp4%' or p.media_url ilike '%.mov%' or p.media_url ilike '%.m4v%'
